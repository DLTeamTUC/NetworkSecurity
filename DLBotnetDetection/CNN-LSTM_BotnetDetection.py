...
Xtrain = Xtrain.reshape((Xtrain.shape[0], 1, 17, 1))
Xtest = Xtest.reshape((Xtest.shape[0], 1, 17, 1))

def CNN_LSTM_model():
    model = Sequential()
    model.add(TimeDistributed(Conv1D(filters=8, kernel_size=2, activation='relu', padding='same'), batch_input_shape=(None, None, 17, 1)))  #5 time steps and 1 features per step    
...
plt.title('Model Accuracy for CNN-LSTM')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
